...
const BUNDLED_YTDLP = path.resolve(__dirname, 'bin', process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp');
const YTDLP_PATH = path.join(DOWNLOAD_DIR, process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp');
...
function rateLimit(maxRequests = 20) {
  return (req, res, next) => {
    ...
    const recent = (requestLog.get(key) || []).filter(ts => now - ts < 60_000);
...
function isYouTubeUrl(rawUrl) {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase().replace(/^www\./, '');
    return ['youtube.com', 'youtu.be', 'm.youtube.com', 'music.youtube.com'].includes(host);
  } catch {
    return false;
  }
}
...
function sanitizeError(message) {
  ...
  if (/private video|video unavailable|members-only|login required/i.test(text)) {
...
async function ensureYtDlp() {
  ...
  if (!asset) throw new Error(`Unsupported CPU architecture for yt-dlp: ${process.arch}`);
...
function ytDlpExecutable() {
  return process.env.YTDLP_PATH || (fs.existsSync(BUNDLED_YTDLP) ? BUNDLED_YTDLP : YTDLP_PATH);
}
...
function normalizeInfo(info, type) {
  ...
  for (const fmt of allFormats) {
    ...
    if ((!previousMuxed && currentMuxed) || (previousMuxed === currentMuxed && currentSize > previousSize)) {
      byHeight.set(key, fmt);
    }
  }

  const sortedVideoFmts = [...byHeight.values()].sort((a, b) => (b.height || 0) - (a.height || 0));
...
async function getInfo(url, type) {
  ...
  if (!stdout.trim()) throw new Error(stderr || 'No media information returned');
...
async function findFinalFile(prefix) {
  ...
  for (const name of entries) {
    if (!name.startsWith(prefix) || name.endsWith('.part') || name.endsWith('.ytdl')) continue;
    const filePath = path.join(DOWNLOAD_DIR, name);
    try {
      const stat = await fsp.stat(filePath);
      if (stat.isFile() && stat.size > 0) files.push({ filePath, size: stat.size });
    } catch {}
  }
...
async function cleanupJobFiles(job) {
  ...
  await Promise.all(entries
    ...
    .map(name => fsp.rm(path.join(DOWNLOAD_DIR, name), { force: true }).catch(() => {})));
}
...
function startYtDlpJob(job, params) {
  ...
  setJob(job, { status: 'downloading', stage: 'Downloading...', progress: 0, startedAt: Date.now() });
  ...
  const timeout = setTimeout(() => {
    ...
    setJob(job, { status: 'error', error: 'Download timed out. Please try a smaller video or try again later.' });
  }, DOWNLOAD_TIMEOUT_MS);
  ...
  const handleLine = line => {
    ...
    if (progress !== null) setJob(job, { progress, stage: progress >= 100 ? 'Processing...' : 'Downloading...' });
    if (/\[Merger\]|Merging formats/i.test(clean)) setJob(job, { status: 'merging', stage: 'Merging video + audio...', progress: Math.max(job.progress || 0, 99) });
    if (/\[ExtractAudio\]|Deleting original file/i.test(clean)) setJob(job, { status: 'processing', stage: 'Finalizing file...', progress: 99 });
  ...
  child.on('close', async code => {
    ...
    if (code !== 0) {
      ...
      setJob(job, { status: 'error', error: sanitizeError(diagnostic || `yt-dlp exited with code ${code}`) });
      activeJobs.delete(job.id);
    ...
    try {
      setJob(job, { status: 'processing', stage: 'Finalizing file...', progress: 99 });
      const outFile = await findFinalFile(prefix);
      if (!outFile) throw new Error('Downloaded file was not produced by yt-dlp.');

      const stat = await fsp.stat(outFile);
      if (!stat.isFile() || stat.size <= 0) throw new Error('Downloaded file is empty.');
...
app.post('/api/info', rateLimit(20), async (req, res) => {
  ...
  if (!url || !isAllowedMediaUrl(url)) {
    return res.status(400).json({ error: 'Please use a valid public Instagram or YouTube URL.' });
  }
  ...
  try {
    ...
    return res.status(500).json({ error: sanitizeError(err.stderr || err.message) });
  }
});
...
app.post('/api/download/start', rateLimit(5), async (req, res) => {
  ...
  if (!url || !isAllowedMediaUrl(url)) return res.status(400).json({ error: 'Unsupported or invalid media URL.' });
  if (!['video', 'audio'].includes(mediaType)) return res.status(400).json({ error: 'Invalid media type.' });
  if (type === 'instagram' && !isInstagramUrl(url)) return res.status(400).json({ error: 'Please use an Instagram URL.' });
  if (type === 'youtube' && !isYouTubeUrl(url)) return res.status(400).json({ error: 'Please use a YouTube URL.' });
  if (activeJobs.size >= MAX_ACTIVE_JOBS) return res.status(429).json({ error: 'The downloader is busy. Please try again in a moment.' });
...
app.get('/api/download/status/:jobId', rateLimit(60), (req, res) => {
  ...
  if (job.expiresAt < Date.now() && job.status !== 'downloading' && job.status !== 'processing' && job.status !== 'merging') {
    return res.status(404).json({ error: 'Download job expired.' });
  }
  return res.json(jobPublic(job));
});

app.get('/api/download/file/:jobId', rateLimit(10), async (req, res) => {
  const job = jobs.get(req.params.jobId);
  if (!job || job.status !== 'ready' || !job.filePath) return res.status(404).json({ error: 'Download file is not ready or has expired.' });
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${job.filename.replace(/"/g, '_')}"; filename*=UTF-8''${encodeURIComponent(job.filename)}`);
    res.setHeader('X-Content-Type-Options', 'nosniff');

    const stream = fs.createReadStream(job.filePath);
    stream.on('error', err => {
      console.error('[api/download/file]', err);
      if (!res.headersSent) res.status(500).json({ error: 'Could not read the download file.' });
      else res.destroy(err);
    });
    stream.on('close', () => {
      if (job.filePath) fsp.rm(job.filePath, { force: true }).catch(() => {});
      jobs.delete(job.id);
    });
    stream.pipe(res);
  } catch (err) {
...
setInterval(async () => {
  ...
  for (const [id, job] of jobs) {
    const running = ['queued', 'downloading', 'merging', 'processing'].includes(job.status);
