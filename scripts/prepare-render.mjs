...
if (await exists(bgutilBuild)) {
  ...
  const bgutilNodeModules = path.join(bgutilServer, "node_modules");
  if (!(await exists(path.join(bgutilNodeModules, "commander")))) {
    console.log("bgutil provider dependencies are missing; installing them...");
    await runCommand("npm", ["ci", "--omit=dev"], { cwd: bgutilServer });
  }
} else {
